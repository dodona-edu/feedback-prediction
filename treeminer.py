        ...
        for x, i, scope_list_x in clazz:
            ...
            for y, j, scope_list_y in clazz:
                ...
                if i == j:
                    for elem1 in scope_list_x:
                        for elem2 in scope_list_y:
                            if (
                                elem1 != elem2
                                and elem1[0] == elem2[0]
                                and elem1[1][-1][0] <= elem2[1][-1][0]
                                and elem1[1][-1][1] >= elem2[1][-1][1]
                            ):
                                ...
                                for k in range(len(to_add) - 1, -1, -1):
                                    if to_add[k][1] == elem2[1]:
                                        if (
                                            to_add[k][0][0] <= elem1[1][0]
                                            and to_add[k][0][1] >= elem1[1][1]
                                        ):
                                            del to_add[k]
                                        elif (
                                            elem1[1][0] <= to_add[k][0][0]
                                            and elem1[1][1] >= to_add[k][0][1]
                                        ):
                                            should_add = False
                                if should_add:
                                    path = [
                                        e for e in elem1[1] if e[1] >= elem2[1][-1][1]
                                    ] + [e for e in elem2[1] if e[1] >= elem2[1][-1][1]]
                    ...
                    if (
                        len(set(x[0] for x in scope_list)) / self.tree_count
                        >= self.support
                    ):
                ...
                for elem1 in scope_list_x:
                    for elem2 in scope_list_y:
                        if elem1 != elem2 and elem1[0] == elem2[0]:
                            ...
                            if (
                                j + 1 < len(elem1[1])
                                and elem1[1][j][0] <= elem2[1][-1][0]
                                and elem1[1][j][1] >= elem2[1][-1][1]
                                and elem1[1][j + 1][1] < elem2[1][-1][0]
                            ):
                                to_be_added = (
                                    elem1[0],
                                    elem1[1][: j + 1] + [elem2[1][-1]],
                                )
                            elif (
                                j < len(elem1[1])
                                and elem1[1][j][1] < elem2[1][-1][0]
                                and elem1[1][j][0] <= elem2[1][j][0]
                                and elem1[1][j][1] >= elem2[1][j][1]
                            ):
                                to_be_added = (
                                    elem1[0],
                                    elem2[1][: j + 1] + [elem2[1][-1]],
                                )
                            if to_be_added is not None:
                                should_add = True
                                for k in range(len(scope_list) - 1, -1, -1):
                                    if (
                                        scope_list[k][0] == to_be_added[0]
                                        and scope_list[k][1][-1] == to_be_added[1][-1]
                                        and scope_list[k][1][-2][0]
                                        > to_be_added[1][-2][0]
                                    ):
                                        should_add = False
                                    if (
                ...
                if (
                    len(set(x[0] for x in scope_list))
                ) / self.tree_count >= self.support:
                            ...
                            if (
                                ...
                                for i in range(len(to_add) - 1, -1, -1):
                                    if to_add[i][1] == elem2[1]:
                                        if (
                                            to_add[i][0][0] <= elem1[1][0]
                                            and to_add[i][0][1] >= elem1[1][1]
                                        ):
                                            del to_add[i]
                                        elif (
                                            elem1[1][0] <= to_add[i][0][0]
                                            and elem1[1][1] >= to_add[i][0][1]
                                        ):
                                            should_add = False
                                if should_add:
...
class Treeminer:
    ...
    def __init__(self, database, support=0.9):
        ...
        for pattern in self.f_1:
            self.frequent_patterns.add((pattern,))
        self.label2id = {el: i for i, el in enumerate(self.f_1)}
        self.support = support
                ...
                if i == j:
                    ...
                    for elem1 in scope_list_x:
                        for elem2 in scope_list_y:
                            ...
                            ) and (elem1[0], elem1[1] + [elem1[2][1]], elem2[2]) not in scope_list:
                                scope_list.append(
                                    (elem1[0], elem1[1] + [elem1[2][1]], elem2[2])
                                )
                    if (
...
if __name__ == "__main__":
    ...
    def analyze_trees(trees):
        ...
        print(t_patterns)
        print()
        assert t_patterns == td_patterns, f"difference in patterns for {trees}: {td_patterns - t_patterns} {t_patterns - td_patterns} {len(t_patterns)} {len(td_patterns)}"
