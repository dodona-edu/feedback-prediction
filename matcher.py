...
def most_likely_messages(pattern_collection, subtree):
    ...
    def match_percentage(m):
        matches_test = list(filter(lambda pattern: subtree_matches_2(subtree, pattern), pattern_collection[m]))
        matches = len(matches_test)
        # matches = len(list(filter(lambda pattern: subtree_matches_2(subtree, pattern), pattern_collection[m])))
        total = len(pattern_collection[m])
        return matches / total, f"{matches}/{total}"
...
def result_for_file(patterns, messages, n=5):
    total = Counter()
    ...
    for m, line in messages[1]:
        ...
        i = list(map(lambda x: x[0], matched)).index(m)
        if i < n:
            first_n[i][m] += 1
...
def test_all_files(test, patterns, n=5):
    results = pqdm(map(lambda i: (patterns, i, n), test.values()), result_for_file, n_jobs=8, argument_type='args')
    ...
    for m in total:
        result = (m, first_n[0][m] / total[m], total_first_n[m] / total[m])
        print(result)

    return total, first_n, total_first_n


def test_one_file(test, patterns):
    start = datetime.datetime.now()
    result = result_for_file(patterns, test)
    print(f"Elapsed time: {datetime.datetime.now() - start}")
    print(result)


def main(test_file=None):
    training, test, patterns = analyze()

    print("Testing...")
    if test_file is None:
        test_all_files(test, patterns)
    else:
        test = test[test_file]
        test_one_file(test, patterns)
